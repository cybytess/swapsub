def load(path):
    ...
    if path.split(".")[len(path.split(".")) - 1] == "lrc":
        ...
        for n in sub:
            ...
            ss_starts.append(str(time.split(':')[1].split('.')[0]).rjust(2, '0'))
            mms_starts.append(str(time.split(':')[1].split('.')[1]).ljust(3, '0'))
            content_list.append(content)
            
            if a == len(sub)-1:
                timenext = sub[a].strip("[").split("]")[0]
                ss_ends.append(str(int(timenext.split(':')[1].split('.')[0])+3).rjust(2, '0'))
            else:               
                timenext = sub[a+1].strip("[").split("]")[0]
                ss_ends.append(str(timenext.split(':')[1].split('.')[0]).rjust(2, '0'))
            hh_ends.append(str(int(timenext.split(':')[0]) // 60).rjust(2, '0'))
            mm_ends.append(str(int(timenext.split(':')[0]) % 60).rjust(2, '0'))
            mms_ends.append(str(timenext.split(':')[1].split('.')[1]).ljust(3, '0'))
        ...
        for a in range(len(hh_starts)):
            ...
            file.write(
                str(int(hh_starts[a]) + 0) + ":" + str(mm_starts[a]) + ":" + str(ss_starts[a]) + "." + str(
                    mms_starts[a]) + "," +
                str(int(hh_ends[a]) + 0) + ":" + str(mm_ends[a]) + ":" + str(ss_ends[a]) + "." + str(mms_ends[a])
